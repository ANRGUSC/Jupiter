...
def gen_stream_fixed_set_data(task_name,interval,num_files,data_path,original_data_path):
    index = task_name.strip('datasource') # take the task_name index number by stripping out the 'datasource' prefix
    list_files = os.listdir(original_data_path)
    list_files = sorted(list_files, key=lambda x: x.split('_')[2].split('botnet')[1])
...
def task(q, pathin, pathout, task_name):
    ...
    time.sleep(30) #waiting for all the pods to be ready
    gen_stream_fixed_set_data(task_name,dummy.STREAM_INTERVAL,dummy.NUM_FILES,data_path,original_data_path)
