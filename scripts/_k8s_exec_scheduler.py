...
def check_status_exec_profiler_workers(app_name):
    ...
    for key in nodes:
        ...
        # the name = key in the respective namespac    # Check if there is a replicaset running by using the label app={key}
        # The label of kubernets are used to identify replicaset associate to each task
        label = "app=" + app_name+'-' + key + "exec-profiler"
...
def k8s_exec_scheduler(app_name):
    ...
    for i in nodes:
        ...
        if i != 'home':
            pod_name = app_name+"-"+i
            body = write_exec_service_specs(name = pod_name, label = pod_name + "exec-profiler")
    ...
    for i in nodes:
        ...
        if i != 'home':
            ...
            """
            pod_name = app_name+"-"+i
            dep = write_exec_specs(name = pod_name, label = pod_name + "exec-profiler", node_name = i, image = jupiter_config.EXEC_WORKER_IMAGE,
