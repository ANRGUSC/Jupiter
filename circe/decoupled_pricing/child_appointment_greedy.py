...
"""
__author__ = "Quynh Nguyen, Pranav Sakulkar,  Jiatong Wang, Pradipta Ghosh, \
              Bhaskar Krishnamachari"
__copyright__ = "Copyright (c) 2019, Autonomous Networks Research Group. All \
                 rights reserved."
__license__ = "GPL"
__version__ = "3.0"
...
def prepare_global():
    ...
    global network_map, FLASK_SVC, MONGO_SVC_PORT, nodes, node_count
    global master_host, debug, FLASK_PORT
    ...
    global threshold, resource_data, is_resource_data_ready
    global network_profile_data, is_network_profile_data_ready, application
...
def init_task_topology():
    ...
    logging.debug("control_relation %s", control_relation)


def assign_task():
    """Request assigned node for a specific task, write task assignment in local
       file at ``local_responsibility/task_name``.

    Raises:
        Exception: ``ok`` if successful, ``not ok`` if either the request or the
        writing is failed
    """
    try:
        ...
        if len(control_relation[task_name]) > 0:
            logging.debug('I am responsible for the next children tasks')
...
def trigger_restart():
    try:
        logging.debug('Trigger retart')
        time_info = request.args.get('trigger_restart')
        logging.debug('Delete all local information')
        local_mapping.clear()
...
def call_send_mapping(mapping, node):
    ...
    try:
        ...
        logging.debug(e)
        logging.debug("Announce the mapping to the master host failed")
        return 'not ok'
    return 'ok'
...
def assign_children_task(children_task):
    ...
    if not assign_to_node:
        logging.debug("No suitable node found for assigning task: %s", children_task)
    else:
        logging.debug("Trying to assign %s to %s", children_task,  assign_to_node)
...
def get_resource_data_drupe(MONGO_SVC_PORT):
    ...
    for profiler_ip in profiler_ips:
        ...
        for record in logdb:
            resource_data[network_map[profiler_ip]]={'memory':record['memory'],
            'cpu': record['cpu'], 'last_update': record['last_update']}
...
def get_network_data_drupe(my_profiler_ip, MONGO_SVC_PORT, network_map):
    ...
    client_mongo = MongoClient('mongodb://' + my_profiler_ip + ':' +
                               MONGO_SVC_PORT + '/')
    ...
    while num_nb == -1:
        ...
        collection = db.collection_names(include_system_collections=False)
        num_nb = len(collection) - 1
    logging.debug('--- Number of neighbors: ' + str(num_nb))
...
def get_network_data_mapping():
    """Mapping the chosen TA2 module (network monitor) based on
       ``jupiter_config.PROFILER`` in ``jupiter_config.ini``
...
def get_resource_data_mapping():
    """Mapping the chosen TA2 module (resource monitor) based on
       ``jupiter_config.PROFILER`` in ``jupiter_config.ini``
...
def schedule_update_profiling(interval):
    ...
    sched.add_job(get_network_data, 'interval',
                  [my_profiler_ip, MONGO_SVC_PORT, network_map],
                  id='network_profiling', minutes=interval,
                  replace_existing=True)
    sched.add_job(get_resource_data, 'interval', [MONGO_SVC_PORT],
                  id='resource_profiling', minutes=interval,
                  replace_existing=True)
    sched.start()
...
def main():
    """
        ...
        - Initialize folders ``local`` and ``local_responsibility``, prepare 
          ``local_children`` and ``local_mapping`` file.
    ...
    DIRECTORY_TO_WATCH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      'task_responsibility')
