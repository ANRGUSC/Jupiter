...
def recv_mapping():
    ...
    try:
        ...
        logging.debug("Received flask message:", worker_node, msg, ts)
        if msg == 'start':
            start_time[worker_node].append(ts)
        else:
            end_time[worker_node].append(ts)
            if worker_node in last_tasks:
                logging.debug("Start time stats:", start_time)
                logging.debug("End time stats:", end_time)
...
def return_output_files():
    ...
    logging.debug("Received request for number of output files. Current done:", num_files)
...
class Handler1(pyinotify.ProcessEvent):
    ...
    def process_IN_CLOSE_WRITE(self, event):
        ...
        logging.debug("execution time is: ", exec_times)
...
class Handler(pyinotify.ProcessEvent):
    ...
    def process_IN_CLOSE_WRITE(self, event):
        ...
        logging.debug("start time is: ", start_times)
...
def main():
    ...
    logging.debug("TASK: ", dag_info[0])
    logging.debug("DAG: ", dag_info[1])
    logging.debug("HOSTS: ", dag_info[2])
