...
def exp8():
    ...
    # folder_list= ['makespan']
    # path_list = [os.path.join(exp8_path,x) for x in folder_list]
    # for path in path_list:
    #     stats[path] = defaultdict(list)
    #     avg_stats[path] = dict()
    #     for (dirpath, dirnames, filenames) in os.walk(path):
    #         for filename in filenames:
    #             tmp = filename.split('/')[-1]
    #             pricing_option = tmp.split('_')[1]
    #             N = tmp.split('_')[2].split('N')[1]
    #             M = tmp.split('_')[3].split('.')[0].split('M')[1]
    #             fname = os.path.join(dirpath,filename)
    #             key = (pricing_option,N,M)
    #             with open(fname,'r') as f:
    #                 lines = f.readlines()
    #                 for line in lines:
    #                     l = line.split(' ')
    #                     app_name = l[1]   
    #                     stats[path][key].append(float(l[-1]))
    #     for key in stats[path]:
    #         avg_stats[path][key] = np.mean(stats[path][key])
        
    #     print(avg_stats[path])

    folder_list= ['msg_overhead']
    path_list = [os.path.join(exp8_path,x) for x in folder_list]
    for path in path_list:
        stats[path] = defaultdict(list)
        avg_stats[path] = dict()
