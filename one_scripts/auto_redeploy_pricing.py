...
from k8s_pricing_jupiter_deploy import *
import datetime
...
def get_pod_logs(namespace, pod_name):
    ...
    """
    ts = int(time.time())
    log_file = "../logs/greedy_both_dummy_circehome_%d.log" %(ts)
    bash_log = "kubectl logs %s -n %s > %s"%(pod_name,namespace,log_file)
    os.system(bash_log)
    # log_runtime = "../logs/dummy_runtime_%d.log" %(ts)
...
def main():
    ...
    with open("../logs/dummy_evaluation_log","a+") as f:
