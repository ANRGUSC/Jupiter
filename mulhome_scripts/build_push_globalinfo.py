...
def build_push_globalinfo():
    """Build globalinfoer home image from Docker files and push them to the Dockerhub.
    """

    port_list_home = prepare_global_info()
    import info_docker_files_generator as dc 

    os.chdir(jupiter_config.GLOBALINFO_PATH)

    home_file = dc.write_globalinfo_home_docker(username = jupiter_config.USERNAME,
                      password = jupiter_config.PASSWORD,
                      app_file = jupiter_config.APP_NAME,
                      ports = " ".join(port_list_home))

    #--no-cache
    cmd = "sudo docker build -f %s ../.. -t %s"%(home_file,jupiter_config.GLOBALINFO_IMAGE)
    os.system(cmd)
    os.system("sudo docker push " + jupiter_config.GLOBALINFO_IMAGE )

    # os.system("rm *.Dockerfile")
if __name__ == '__main__':
    build_push_globalinfo()
