...
def write_decoupled_pricing_circe_service_specs(**kwargs):
    """
    ...
    jupiter_config.set_globals()
    # insert your values
    specific_yaml = template_decoupled.format(flask_svc = jupiter_config.FLASK_SVC,
                                    flask_port = jupiter_config.FLASK_DOCKER,
                                    **kwargs)
    dep = yaml.load(specific_yaml)
    return dep
...
def write_decoupled_pricing_circe_compute_service_specs(**kwargs):
    """
    This function genetares the service description yaml for CIRCE
    ...
    config.read(INI_PATH)
    # insert your values
    specific_yaml = template_compute_decoupled.format(ssh_svc = jupiter_config.SSH_SVC,
                                    ssh_port = jupiter_config.SSH_DOCKER, 
                                    flask_svc = jupiter_config.FLASK_SVC,
                                    flask_port = jupiter_config.FLASK_DOCKER,
                                    mongo_svc = jupiter_config.MONGO_SVC,
