...
def k8s_decoupled_pricing_circe_scheduler(dag_info ,profiler_ips, execution_ips,app_name):
    ...
    for key in homes:
        ...
        home_service_ips[key] = resp.spec.cluster_ip

    all_home_ips = ':'.join(home_service_ips.values())
    all_home = ':'.join(home_service_ips.keys())
    ...
    for i in nodes:
        ...
        if i != 'home':
            ...
            body = write_circe_service_specs(name = pod_name, label = pod_label)
    ...
    all_controller_ips = ':'.join(service_ips.values())
    all_controller = ':'.join(service_ips.keys())
    ...
    for i in nodes:
        ...
        if i != 'home':
            ...
            """
                Generate the yaml description of the required deployment for CIRCE controller workers
            """
            pod_name = app_name+'-controller'+i
            label_name = app_name+'-controller'+i
            dep = write_decoupled_circe_controller_specs(name = pod_name, label = label_name, image = jupiter_config.WORKER_CONTROLLER_IMAGE,
                                             host = nodes[i][0], all_controller = all_controller,
                                             all_controller_ips = all_controller_ips,
                                             self_name=i,
                                             home_ip = all_home_ips,
                                             home_name = all_home,
    ...
    for key in homes:
        ...
        home_dep = write_decoupled_circe_home_specs(name = home_name, label = label_name,
                                image = jupiter_config.PRICING_HOME_IMAGE, 
                                host = jupiter_config.HOME_NODE, all_controller = all_controller,
                                             all_controller_ips = all_controller_ips,
                                             self_name = home_name,
                                             home_ip = all_home_ips,
                                             home_name = all_home,
                                             serv_ip = home_service_ips[key],
    ...
    #     pod_name = app_name+"-"+i
    #     #print(pod_name)
    #     dep = write_decoupled_circe_computing_specs(name = pod_name, label =  pod_name, image = jupiter_config.WORKER_COMPUTING_IMAGE,
