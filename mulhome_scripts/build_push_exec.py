__author__ = """Pradipta Ghosh, Pranav Sakulkar, Quynh Nguyen, Jason A Tran,
...
def prepare_global_info():
    ...
    Returns:
        - list: port_list_home - The list of ports to be exposed in the exec
                                    home dockers
        - list: port_list_worker - The list of ports to be exposed in the exec
                                    worker dockers
    """
    ...
    logging.debug('The list of ports to be exposed in the exec home dockers',
                  'are {}'.format(" ".join(port_list_home)))
    ...
    logging.debug('The list of ports to be exposed in the exec worker dockers',
                  'are {}'.format(" ".join(port_list_worker)))

    return port_list_home, port_list_worker


def build_push_exec():
    """Build execution profiler home and worker image from Docker files and
       push them to the Dockerhub.
    """
    ...
    home_file = dc.write_exec_home_docker(
    ...
    worker_file = dc.write_exec_worker_docker(
        username=jupiter_config.USERNAME,
        password=jupiter_config.PASSWORD,
        app_file=jupiter_config.APP_NAME,
    ...
    cmd = "sudo docker build -f {} ../.. -t {}".format(
        home_file, jupiter_config.EXEC_HOME_IMAGE)
    os.system(cmd)
    os.system("sudo docker push " + jupiter_config.EXEC_HOME_IMAGE)

    cmd = "sudo docker build -f {} ../.. -t {}".format(
        worker_file, jupiter_config.EXEC_WORKER_IMAGE)
    os.system(cmd)
