...
### decoupled pricing
template_home_controller_decoupled = """
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {name}
...
def write_decoupled_pricing_controller_home_specs(**kwargs):
    ...
    specific_yaml = template_home_controller_decoupled.format(flask_port = jupiter_config.FLASK_DOCKER,
                                    **kwargs)
    dep = yaml.load(specific_yaml)
    return dep

template_worker_controller_decoupled = """
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {name}
...
def write_decoupled_pricing_controller_worker_specs(**kwargs):
    ...
    specific_yaml = template_worker_controller_decoupled.format(flask_port = jupiter_config.FLASK_DOCKER,
                                    **kwargs)
    dep = yaml.load(specific_yaml)
    return dep


template_compute_worker_decoupled = """
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {name}
...
def write_decoupled_circe_compute_worker_specs(**kwargs):
    ...
    specific_yaml = template_compute_worker_decoupled.format(flask_port = jupiter_config.FLASK_DOCKER,
                                              ssh_port = jupiter_config.SSH_DOCKER,
                                    **kwargs)
    dep = yaml.load(specific_yaml)
    return dep

template_compute_home_decoupled = """
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {name}
...
def write_decoupled_circe_compute_home_specs(**kwargs):
    ...
    specific_yaml = template_compute_home_decoupled.format(ssh_port = jupiter_config.SSH_DOCKER, 
                                    flask_port = jupiter_config.FLASK_DOCKER,
                                    mongo_port = jupiter_config.MONGO_DOCKER,
                                    **kwargs)
