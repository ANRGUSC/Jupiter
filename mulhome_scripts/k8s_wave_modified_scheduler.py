...
def extract_geo_info(nodes):
    geo_nodes = dict()   
    for node, fullname in nodes.items():
        geo = fullname[0].split('-')[-1][0:3]
        if geo not in geo_nodes:
            geo_nodes[geo] = [node]
        else:
...
def k8s_wave_modified_scheduler(profiler_ips,app_name):
    """
    ...
    pprint(nodes)
    all_node_geo = extract_geo_info(nodes)
    print(all_node_geo)
    ...
    print(profiler_ips)
    print(service_ips)
    home_dep = write_wave_modified_specs(name = home_name, label = label_name,
