...
class Duplication:
    ...
    def get_task_by_id(self, tasks, num):
        for tk in tasks:
            if tk.number == num:
                return tk
    ...
    def duplicate(self, links, processors, tasks, comp_cost, data, quaratic_profile, btnk_id, new_node,
        ...
        """
        tasks_to_dup = [self.get_task_by_id(tasks, i) for i in task_ids_to_dup]
        tasks_to_recv = [self.get_task_by_id(tasks, i) for i in task_ids_to_recv]
        src_proc = self.get_proc_by_id(processors, btnk_id.split('_')[0])
        dst_proc = self.get_proc_by_id(processors, btnk_id.split('_')[1])
        ...
        if len(file_sizes) > 0:
            procnum = self.get_task_by_id(tasks, parent_ids[idx]).processor_num
