"""
This module is the HEFT algorithm modifed based on repo below by Ouyang Liduo.

    https://github.com/8da2k/heft
"""
...
class HEFT:
    ...
    def __init__(self, filename, node_info):
        ...
        self.num_task, self.task_names, self.num_processor, comp_cost, self.rate, self.data, self.quaratic_profile = create_input.init(filename, node_info)
    ...
    def run(self, task_mapper):
        for task in self.tasks:
            if task == self.tasks[0]:
                ...
                elif task_mapper == "heft_balanced":
                    for processor in self.processors:
                        est = self.cal_est(task, processor)
    ...
    def display_result(self):
        ...
        """
        for t in self.tasks:
            log.info('task %d : up_rank = %f, down_rank = %f' % (t.number, t.up_rank, t.down_rank))
    ...
    def output_assignments(self):
        """Output the scheduling and corresponding assignments to
